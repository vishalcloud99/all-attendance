...
/**
 * Generate a device fingerprint using 14 browser properties hashed with FNV-1a 32-bit.
 * Result format: "DEV-A3F7C291"
 * Synchronous. Falls back to 'DEV-UNKNOWN' on any error.
 */
function generateFingerprint() {
    try {
        ...
        return 'DEV-' + hash.toString(16).toUpperCase().padStart(8, '0');
    } catch (e) {
        return 'DEV-UNKNOWN';
    }
}
 ...
 * Stores fingerprint under 'device_fp' key. Auto-fills saved preferences.
 */
function loadUserInfo() {
    // Always regenerate on load (deterministic) — but cache in AppState
    const fp = generateFingerprint();
    AppState.fingerprint = fp;

    // Show fingerprint badge in header
    const badge = document.getElementById('fp-badge');
    if (badge) badge.textContent = fp;

    const name = localStorage.getItem('pref_' + fp + '_name') || localStorage.getItem('employee_name');
    const role = localStorage.getItem('pref_' + fp + '_role') || localStorage.getItem('employee_role');

    if (name && role) {
        AppState.employeeName = name;
        AppState.role = role;
        // Migrate to new key format if needed
        localStorage.setItem('pref_' + fp + '_name', name);
        localStorage.setItem('pref_' + fp + '_role', role);
...
function renderBottomNav() {
    // All roles see all dashboard action buttons
...
function setupEventListeners() {
    ...
    if (setupForm) {
        // Auto-fill setup fields from saved preferences
        const savedName = localStorage.getItem('pref_' + AppState.fingerprint + '_name');
        const savedRole = localStorage.getItem('pref_' + AppState.fingerprint + '_role');
        ...
        if (savedRole && roleInput) {
            ...
            roleInput.style.borderColor = '#22c55e';
        }
        // Save on change
        if (nameInput) nameInput.addEventListener('input', () => {
            nameInput.style.borderColor = '';
            localStorage.setItem('pref_' + AppState.fingerprint + '_name', nameInput.value.trim());
        });
        if (roleInput) roleInput.addEventListener('change', () => {
            roleInput.style.borderColor = '';
            localStorage.setItem('pref_' + AppState.fingerprint + '_role', roleInput.value);
        });
        ...
        setupForm.addEventListener('submit', (e) => {
            ...
            if (name && role) {
                // Save under fingerprint-namespaced keys
                localStorage.setItem('pref_' + AppState.fingerprint + '_name', name);
                localStorage.setItem('pref_' + AppState.fingerprint + '_role', role);
...
function prefillForms() {
    ...
    // Set hidden fingerprint fields in all forms
    ['hidFingerprint-ga', 'hidFingerprint-sv', 'hidFingerprint-sa', 'hidFingerprint-ex'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = AppState.fingerprint;
    });
}
...
async function handleExpenseEntry(e) {
    ...
    try {
        ...
        const payload = {
            deviceFingerprint: AppState.fingerprint,
...
function renderHistory() {
    ...
    const filtered = historyData.filter(item => {
        ...
        if (filter !== 'all' && item.date) {
            const now = new Date();
            const todayStr = Utils.getLocalDateStr(now);
            if (filter === 'today') {
                matchesDate = item.date === todayStr;
            } else if (filter === 'week') {
                const parts = item.date.split('-');
                const itemLocal = new Date(+parts[0], +parts[1]-1, +parts[2]);
                const diff = (now - itemLocal) / (1000 * 60 * 60 * 24);
                matchesDate = diff <= 7;
            } else if (filter === 'month') {
                const [y, m] = item.date.split('-');
                matchesDate = +m - 1 === now.getMonth() && +y === now.getFullYear();
            }
        }
    ...
    filtered.forEach(item => {
        let badge = '', receiptBtn = '';
        const formattedDate = Utils.formatDateTime(item.date, item.time);

        // Generate prefixed record ID in format: PREFIX+DDMMYYYYHHMMSS
        let idPrefix = 'TX';
        if (currentHistoryType === 'general') idPrefix = 'AD';
        else if (currentHistoryType === 'service') idPrefix = 'SV';
        else if (currentHistoryType === 'visits') idPrefix = 'VS';
        const recordId = item.id || Utils.generateId(idPrefix, item.date, item.time);

        if (currentHistoryType === 'general') {
            // Attendance card: show the 3 requested fields only
            html += `
                <div class="history-card">
                    <div class="history-card-header">
                        <span style="font-size:0.72rem;color:#888;font-family:monospace;">${recordId}</span>
                    </div>
                    <div class="hist-field-row">
                        <span class="hist-field-label"><i class="fa-regular fa-clock"></i> Date &amp; Time</span>
                        <span class="hist-field-value">${formattedDate}</span>
                    </div>
                    <div class="hist-field-row">
                        <span class="hist-field-label"><i class="fa-solid fa-location-dot"></i> Location</span>
                        <span class="hist-field-value">${item.location || '—'}</span>
                    </div>
                    <div class="hist-field-row">
                        <span class="hist-field-label"><i class="fa-solid fa-bullseye"></i> Purpose</span>
                        <span class="hist-field-value">${item.purpose || '—'}</span>
                    </div>
                </div>
            `;
        } else {
            ...
            if (currentHistoryType === 'visits') {
                ...
                detail = `${formattedDate} • ${item.visitPurpose}`;
            } else if (currentHistoryType === 'service') {
                title = item.siteName || 'Unknown Site';
                detail = `${formattedDate} • Labour: ${item.numberOfLabour}`;
            } else if (currentHistoryType === 'expenses') {
                title = item.category;
                detail = `${formattedDate} • ₹${item.amount}`;
                ...
                if (item.attachment) {
                    receiptBtn = `<a href="${item.attachment}" target="_blank" class="receipt-link" style="display:inline-flex;align-items:center;gap:4px;margin-top:6px;font-size:0.78rem;padding:4px 10px;border-radius:20px;background:#f0f4ff;color:#3b6cf8;text-decoration:none;font-weight:600;"><i class="fa-solid fa-file-invoice"></i> View Receipt</a>`;
                }
            }
            ...
            html += `
                <div class="history-card">
                    <div class="history-card-header">
                        <span>${formattedDate} -- Auto capture</span>
                        ${badge}
                    </div>
                    <div class="history-card-title">${title}</div>
                    <div class="history-card-detail">${detail}</div>
                    <div style="font-size:0.72rem;color:#888;margin-top:4px;font-family:monospace;">${recordId}</div>
                    ${receiptBtn}
                </div>
            `;
        }
...
function renderExpenseHistory(filterType = 'today') {
    ...
    const filtered = userExpenses.filter(item => {
        ...
        const yesterdayStr = Utils.getLocalDateStr(yesterdayDate);
        ...
        if (filterType === 'today') {
            ...
            // Compare local date strings to avoid UTC midnight bug
            const itemParts = item.date.split('-');
            const itemLocal = new Date(+itemParts[0], +itemParts[1]-1, +itemParts[2]);
            const diff = (now - itemLocal) / (1000 * 60 * 60 * 24);
    ...
    filtered.forEach((item, index) => {
        ...
        // Generate TX ID in format: TXDDMMYYYYHHMMSS
        const txId = item.id || Utils.generateId('TX', item.date, item.time);
        const formattedTimestamp = Utils.formatDateTime(item.date, item.time);

        // Show Edit/Delete only if record is from today (EOD restriction)
        const todayLocal = Utils.getLocalDateStr();
        const isEditable = item.date === todayLocal;
        const editBtns = isEditable ? `
            <div class="card-actions">
                <button class="btn-edit" onclick='openEditModal(${JSON.stringify(item)})'>
                    <i class="fa-solid fa-pen"></i> Edit
                </button>
                <button class="btn-delete" onclick="deleteExpense('${txId}')">
                    <i class="fa-solid fa-trash"></i> Delete
                </button>
            </div>` : '';

        html += `
            <div class="transaction-card" id="card-${txId}">
                ...
                <div class="transaction-meta">
                    <i class="fa-regular fa-calendar"></i> ${formattedTimestamp}
                ...
                <div class="transaction-footer">
                    <i class="fa-regular fa-clock"></i> Entry Timestamp: ${formattedTimestamp}
                </div>
                ${editBtns}
            </div>
        `;
...
// ==========================================================================
// Expense Edit / Delete Modal Logic
// ==========================================================================
...
function openEditModal(item) {
    ...
    document.getElementById('edit-tx-id').value = item.id || '';
    document.getElementById('edit-category-hidden').value = category;
    document.getElementById('edit-category-text').textContent = category;
    document.getElementById('edit-amount').value = item.amount || '';
    document.getElementById('edit-description').value = item.description || '';

    const subWrapper = document.getElementById('edit-subcategory-wrapper');
    const pmWrapper  = document.getElementById('edit-payment-wrapper');
    const optGeneral = document.getElementById('edit-opt-general');
    const optAdjust  = document.getElementById('edit-opt-adjustment');

    if (category === 'Advance Received') {
        ...
        if (category === 'Adjustment Against Salary') {
            optGeneral.classList.add('hidden');
            optAdjust.classList.remove('hidden');
        } else {
            optGeneral.classList.remove('hidden');
            optAdjust.classList.add('hidden');
        }
        document.getElementById('edit-sub-category').value = item.subCategory || '';
    }
...
async function submitEditExpense() {
    ...
    const category = document.getElementById('edit-category-hidden').value;
    const amount   = parseFloat(document.getElementById('edit-amount').value);
    const desc     = document.getElementById('edit-description').value;
    const subCat   = document.getElementById('edit-sub-category').value;
    const pm       = document.getElementById('edit-payment-method').value;

    if (!txId)          return UI.showToast('Missing transaction ID.', 'error');
    if (!amount || amount <= 0) return UI.showToast('Please enter a valid amount.', 'error');

    UI.showLoader();
    try {
        const res = await ApiService.post('updateExpense', {
            transactionId: txId, amount, description: desc,
            subCategory: subCat || undefined,
            paymentMethod: pm || undefined
        });
        if (res.status === 'success') {
            const idx = userExpenses.findIndex(e => (e.id || Utils.generateId('TX', e.date, e.time)) === txId);
            if (idx > -1) {
                userExpenses[idx].amount = amount;
                userExpenses[idx].description = desc;
                if (subCat) userExpenses[idx].subCategory = subCat;
                if (pm)     userExpenses[idx].paymentMethod = pm;
            }
            closeEditModal();
            updateExpenseScorecard();
            ...
            UI.showToast('Update failed: ' + res.message, 'error');
        }
    } catch (err) {
        UI.showToast('Error: ' + err.message, 'error');
    } finally {
...
async function deleteExpense(txId) {
    ...
    UI.showLoader();
    try {
        const res = await ApiService.post('deleteExpense', { transactionId: txId });
        if (res.status === 'success') {
            userExpenses = userExpenses.filter(e =>
                (e.id || Utils.generateId('TX', e.date, e.time)) !== txId
            );
            updateExpenseScorecard();
            const activeFilter = document.querySelector('.filter-pill.active')?.getAttribute('data-filter') || 'all';
            renderExpenseHistory(activeFilter);
            ...
            UI.showToast('Delete failed: ' + res.message, 'error');
        }
    } catch (err) {
        UI.showToast('Error: ' + err.message, 'error');
    } finally {
...
const Utils = {
    ...
    getDateTime: () => {
        const now = new Date();
        // Use LOCAL date parts — toISOString() returns UTC which is wrong for IST after midnight
        const date = Utils.getLocalDateStr(now);
    ...
    /**
     * Get local date string in YYYY-MM-DD format (avoids UTC offset bug)
     * @param {Date} [d] defaults to now
     * @returns {string} e.g. '2026-03-22'
     */
    getLocalDateStr: (d) => {
    ...
    /**
     * Format a date string + time string into a human-readable local datetime string.
     * Handles ISO strings (e.g. '2026-03-15T18:30:00.000Z'), plain dates ('2026-03-15'),
     * and plain times ('14:30:00').
     * @param {string} dateStr  e.g. '2026-03-15' or '2026-03-15T18:30:00.000Z'
     * @param {string} timeStr  e.g. '14:30:00' or '2026-03-15T09:00:00.000Z'
     * @returns {string} e.g. '15 Mar 2026, 02:30 PM'
     */
    formatDateTime: (dateStr, timeStr) => {
        try {
            // Normalize date part
            let cleanDate = dateStr || '';
            if (cleanDate.indexOf('T') > -1) cleanDate = cleanDate.split('T')[0];

            // Normalize time part (could be a full ISO string from GAS)
            let cleanTime = timeStr || '00:00:00';
            if (cleanTime.indexOf('T') > -1) {
                // It's a full ISO string — extract the time in local zone
                const t = new Date(cleanTime);
                cleanTime = String(t.getHours()).padStart(2, '0') + ':'
            ...
            if (isNaN(dt.getTime())) return `${cleanDate} ${cleanTime}`;
    ...
    /**
     * Generate a record ID in format: PREFIX + DDMMYYYY + HHMMSS
     * e.g. TX21032026160902, AD21032026091530
     * @param {string} prefix - e.g. 'TX', 'AD', 'SV', 'VS'
     * @param {string} dateStr - 'YYYY-MM-DD'
     * @param {string} timeStr - 'HH:MM:SS'
     * @returns {string}
     */
    generateId: (prefix, dateStr, timeStr) => {
        try {
            // Normalize date
            let cleanDate = (dateStr || '').split('T')[0]; // strip ISO tail
            const [yyyy, mm, dd] = cleanDate.split('-');
            const datePart = `${dd || '00'}${mm || '00'}${yyyy || '0000'}`;

            // Normalize time
            let cleanTime = (timeStr || '000000');
            if (cleanTime.indexOf('T') > -1) {
                const t = new Date(cleanTime);
                cleanTime = String(t.getHours()).padStart(2,'0')
                          + String(t.getMinutes()).padStart(2,'0')
            ...
            const timePart = cleanTime.substring(0, 6).padEnd(6, '0');
